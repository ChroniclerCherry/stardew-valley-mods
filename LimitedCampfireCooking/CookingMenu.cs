...
{
    public class CookingMenu : IClickableMenu
    {
            ...
            : base(x, y, width, height, false)
        {
            ...
            if (ModEntry.Config.EnableAllCookingRecipies)
            {
                foreach (string key in CraftingRecipe.cookingRecipes.Keys)
                    playerRecipes.Add(key);
            } else
            {
                foreach (string key in ModEntry.Config.Recipes)
                {
                    if (CraftingRecipe.cookingRecipes.ContainsKey(key)){
                        playerRecipes.Add(key);
                    } else
                    {
                        ModEntry.monitor.Log($"The cooking recipe for \"{key}\" was not found. It will not be added to the cooking menu.", LogLevel.Info);
                    }
        ...
        private void layoutRecipes(List<string> playerRecipes)
        {
            ...
            foreach (string playerRecipe in playerRecipes)
            {
                ...
                int num5 = 200 + num3;
                ClickableTextureComponent textureComponent = new ClickableTextureComponent("", new Rectangle(num1 + x * (64 + num2), this.craftingPageY() + y * 72, 64, recipe.bigCraftable ? 128 : 64), (string)null, Game1.player.cookingRecipes.ContainsKey(recipe.name) ? "" : "ghosted", recipe.bigCraftable ? Game1.bigCraftableSpriteSheet : Game1.objectSpriteSheet, recipe.bigCraftable ? Game1.getArbitrarySourceRect(Game1.bigCraftableSpriteSheet, 16, 32, recipe.getIndexOfMenuView()) : Game1.getSourceRectForStandardTileSheet(Game1.objectSpriteSheet, recipe.getIndexOfMenuView(), 16, 16), 4f, false);
